...
[Model Serving Endpoints]
   ├─ cosmos-video-encoder   (GPU_SMALL, scale_to_zero) → Cosmos video embedding (768次元) ✅
...
| Model Serving Endpoint | `cosmos-video-encoder` (GPU_SMALL, scale_to_zero) | Cosmos text/video embedding (768次元) |
...
### 問題 17: ウォームアップボタンが GPU 起動完了前に「完了」を表示する

**現象**  
エンドポイント管理パネルの「ウォームアップ」ボタンを押すと、数秒で「完了」と表示される。しかし実際には GPU インスタンスはまだコールドスタート中であり、その直後に検索を実行するとタイムアウトが発生する。

**原因**  
初期実装のポーリング条件が `ep.ready === "READY"` だった。しかし `ready` フィールドはエンドポイントの**設定が有効か**を示すだけで、GPU インスタンスが実際に起動・応答できる状態かを反映しない。`scale_to_zero` でコールド状態であっても常に `"READY"` を返すため、ウォームアップ完了の判定に使えない。

```
[旧] POST /warmup → 即座に 200 返却 → ready=READY をポーリング → 数秒で「完了」
                                                            ↑ GPU はまだコールド
[新] POST /warmup → task_id 返却 → 実際の推論リクエストをバックグラウンド実行
                                   → GET /warmup/{task_id} をポーリング
                                   → status="done"（推論成功）になって初めて「完了」
```

**解決策**  
バックエンドでウォームアップを task_id で管理し、**実際の推論リクエスト (`/invocations`) が成功した**ことをもって完了と判断する:

```python
# Backend: warmup を task として管理
warmup_tasks: dict = {}

def _do_warmup(task_id: str, endpoint_name: str):
    try:
        resp = requests.post(f".../serving-endpoints/{endpoint_name}/invocations", ...)
        resp.raise_for_status()
        warmup_tasks[task_id] = {"status": "done"}   # 推論成功 = GPU 起動済み
    except Exception as e:
        warmup_tasks[task_id] = {"status": "error", "error": str(e)}

@app.post("/api/endpoints/{name}/warmup")
async def warmup_endpoint(name, background_tasks):
    task_id = str(uuid.uuid4())
    warmup_tasks[task_id] = {"status": "pending"}
    background_tasks.add_task(_do_warmup, task_id, name)
    return {"task_id": task_id}         # ← task_id を返す

@app.get("/api/endpoints/warmup/{task_id}")
async def get_warmup_status(task_id):
    return warmup_tasks.get(task_id) or HTTPException(404)
```

```javascript
// Frontend: task_id をポーリング (10秒間隔)
const { task_id } = await fetch(`/api/endpoints/${name}/warmup`, {method:"POST"}).then(r=>r.json());
for (let i = 0; i < 45; i++) {
    await new Promise(r => setTimeout(r, 10000));
    const data = await fetch(`/api/endpoints/warmup/${task_id}`).then(r=>r.json());
    if (data.status === "done") { setWarmupDone(true); break; }   // 推論成功で「完了」
