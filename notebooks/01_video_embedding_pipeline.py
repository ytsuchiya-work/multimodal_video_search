...
# MAGIC
# MAGIC **実行要件**: Databricks Cluster (CPU可。cosmos-video-encoder エンドポイントで推論)

# COMMAND ----------

# MAGIC %pip install decord pillow
# MAGIC # requests is pre-installed
...
# MAGIC %md
# MAGIC ## cosmos-video-encoder エンドポイント設定
# MAGIC
# MAGIC デプロイ済みの cosmos-video-encoder Model Serving エンドポイントを使用して
# MAGIC ビデオ embedding を生成する。

# COMMAND ----------

COSMOS_ENDPOINT = "cosmos-video-encoder"
...
def compute_video_embedding(frames):
    """フレーム列からビデオembeddingを計算 (cosmos-video-encoder エンドポイント経由)"""
    frames_b64 = []
    for frame in frames:
